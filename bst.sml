...
|   find(Node(L,R,K,V), key) =
	if K = key 
		...
		find(L, key) @ find(R,key);

(*leftmost leaf of right subtree*)
fun getSuccessor(Empty) = Empty
|   getSuccessor(Node(Empty, Empty,K,V)) = Node(Empty,Empty,K,V)
|   getSuccessor(Node(Empty,R,_,_)) =  getSuccessor(R)
...
fun postorder(Empty) = []
|   postorder(Node(Empty,Empty,k,v)) = 
	[(0,k,v)]
|   postorder(Node(Empty,R,k,v)) = 
	postorder(R) @ [(2,k,v)]
|   postorder(Node(L,Empty,k,v)) =
	postorder(L) @ [(1,k,v)]
|   postorder(Node(L,R,k,v)) =
	postorder(L) @ postorder(R) @ [(3,k,v)];	
...
|   delete(Node(Empty, R, K, V), key) =
	if K = key
		...
		Node(Empty, delete(R, key), K, V)
|   delete(Node(L, Empty, K, V), key) =
	if K = key 
		then L
	else
		Node(delete(L, key), Empty, K, V)
|   delete(Node(L,R,K,V), key) = 
	if K = key then
		let
			val Node(a,b,c,d) = getSuccessor(R)
		in
			Node(L,delete(R,c),c,d) (*delete successor*)
		end
...
fun subtree(Empty, min, max) = Empty
|   subtree(Node(Empty,Empty,k,v),min,max) =
	if k < min orelse k > max
		then Empty
	else
		Node(Empty,Empty,k,v)
|   subtree(Node(Empty,R,k,v),min,max) =
	if k > max
		then Empty
	else if k < min
		then subtree(R,min,max)
	else
		Node(Empty,subtree(R,min,max),k,v)
|   subtree(Node(L,Empty,k,v),min,max) = 
	if k < min
		then Empty
	else if k > max
		then subtree(L,min,max)
	else
		Node(subtree(L,min,max),Empty,k,v)
